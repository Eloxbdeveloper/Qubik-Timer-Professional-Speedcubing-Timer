...
export const crearCard = (dato, index, modificable) => {
                ...
                if (cursor) {
                    if (contador === index) {
                        ...
                        if (registro.dnf) {
                            ...
                            btnMas2.style.backgroundColor =
                                "rgba(255, 255, 255, 0.05)";
                            timeCard.textContent = registro.timeDNF;
                        } else {
                            btnDNF.style.backgroundColor =
                                "rgba(255, 255, 255, 0.05)";
                            timeCard.textContent = Number(
                                registro.time,
                            ).toFixed(2);
                        }
                    ...
                    if (contador === index) {
                        ...
                        if (registro.masDos) {
                            ...
                            timeCard.textContent =
                                Number(registro.timeMasDos).toFixed(2) + "+";
                        } else {
                            btnMas2.style.backgroundColor =
                                "rgba(255, 255, 255, 0.05)";
                            timeCard.textContent = Number(
    ...
    btnClose.addEventListener("click", (e) => overlayCard.remove());

    document.addEventListener("click", (e) => {
        if (e.target === overlayCard) overlayCard.remove();
    });
