...
export function eventosScramble() {
    ...
    menuScramble[1].addEventListener("click", () => {
        ...
        // Sincroniza la propiedad readOnly de la interfaz basándose en la flag invertida de 'isEditable'
        notacion.readOnly = !isEditable;

        notacion.classList.toggle("borde_textarea");
        if (isEditable)
            notacion.focus(); // Transfiere el foco del teclado directamente al campo de texto
        else notacion.blur(); // Retira el foco del cursor del elemento actual
    });
    ...
    // Detectar clic fuera para desactivar edición
    document.addEventListener("click", (e) => {
        if (!isEditable) return;

        // Verificamos si el clic NO fue en la notación y NO fue en ninguno de los botones del menú
        const clickEnNotacion = notacion.contains(e.target);
        const clickEnBotones = Array.from(menuScramble).some((btn) =>
            btn.contains(e.target),
        );

        if (!clickEnNotacion && !clickEnBotones) {
            isEditable = false;
            notacion.readOnly = true;
